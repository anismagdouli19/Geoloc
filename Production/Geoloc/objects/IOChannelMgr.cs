...
public class IOChannelMgr
{
    ...
    IOChannelMgr()
    {
        ...
        AddIOChannel("EVTCODE", 100);       //Код события
        AddIOChannel("STATUS", 101);

        AddIOChannel("ALARM", 110);         //Тревога

        for (int i = 0; i < 256; i++)       //тревоги
            AddIOChannel("ALARM" + i, (ushort)(i + 2048));
        ...
        for (int i = 0; i < 256; i++)   //OBDII PID
            AddIOChannel("OBDII" + i, (ushort)(i + 1536));
        for (int i = 0; i < 256; i++)   //OBDII ALARMS
            AddIOChannel("OBDII_ALARM" + i, (ushort)(i + 1794));
    }
