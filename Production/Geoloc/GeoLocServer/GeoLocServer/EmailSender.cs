...
public class EmailSender
{
    ...
    void Tick(object obj)
    {
        try
        {
            if (m_Emails.Count > 0)
            {
                SmtpClient client = new SmtpClient("mail.locanix.net",25);
                client.Credentials = new NetworkCredential("alerts","locanix22@456");
    ...
    bool SendMessage(SmtpClient client, Email mail)
    {
        try
        {
            ...
            MailMessage msg = new MailMessage("alerts@locanix.net", addr,mail.m_strSubject,mail.m_strMessage);
            client.Send(msg);
            return true;
        }
        catch (Exception ex)
    ...
    public void AddMessage(string strAddr,string strSubject, string strMessage)
    {
        if (strAddr.Length == 0)
            return;
