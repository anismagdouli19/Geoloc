    ...
    void FlushData(object obj)    //записываем данные в базу данных
    {
        ...
        using (CDatabase db = Configuration.inst().GetDB())
        {
            ...
            try
            {
                ...
                using (MySqlCommand cmdInsert = new MySqlCommand("INSERT INTO points(TrackerID, Time, Lat, Lng, Status, Speed, Alt, IO, GSMInfo) VALUES(?ID, ?Time, ?Lat, ?Lng, ?Status, ?Speed, ?Alt, ?IO, ?GSMInfo) ON DUPLICATE KEY UPDATE TrackerID=TrackerID", db.connection))
                {
